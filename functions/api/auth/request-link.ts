...
export const onRequestPost = async (context: CFContext): Promise<Response> => {
    try {
        ...
        ).bind(context.env.WORKSPACE_ID, email).first();
        ...
        await context.env.DB.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, datetime('now'))
        `).bind(linkId, context.env.WORKSPACE_ID, email, tokenHash, expiresAt).run();

        // Send email via Resend
        const loginUrl = `${new URL(context.request.url).origin}/api/auth/consume?token=${token}`;
